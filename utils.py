...
def make_data_for_model(data_df):
    ...
    for sss in ['Open', 'High', 'Low', 'Close', 'Volume_(BTC)', 'Volume_(Currency)']:
        data_df[sss] = data_df[sss].shift(-1)
    ...
    #  train_data = data_df.loc[str(config.year_start_pred):][use_cols]
...
def scale_data(train_data, test_data):
    ...
    #  feat_cols = config.stats_cols + [config.Label_Price]
    ...
    # 数据标签
    """
    y_train = train_data[config.raw_label_col].values.reshape(-1, 1)
    y_test = test_data[config.raw_label_col].values.reshape(-1, 1)
"""
